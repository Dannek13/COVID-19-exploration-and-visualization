-- COVID-19 Data Exploration / Průzkum dat COVID-19
-- Author / Autor: D.Grinin
-- Description / Popis: 
	-- Full SQL script used for: table creation, data loading, cleaning, exploration, view creation, query preparation for Tableau viz
	-- Kompletní SQL skript pro: vytvoření tabulek, načtení dat z CSV, čištění a kontrolu dat, průzkumné dotazy, tvorbu view a přípravu pro Tableau vizualizace
-- Language / Jazyk: Czech and English
-- MySQL Workbench 8.0

-- 1. TABLE SETUP

-- Nejprve je potřeba vytvořit tabulky pro nahrání dat z CSV, z důvodu velikosti souborů je lepší využít skriptu než rozhraní v MySQL
-- Aby se předešlo případným chybám přidám DROP TABLE IF EXISTS
-- Vytvořím dvě tabulky 'CovidDeaths' a 'CovidVaccinations', propojeny jsou pomocí FOREIGN KEY
...
LOAD DATA LOCAL INFILE '.../path/to/CovidDeaths.csv'
...
LOAD DATA LOCAL INFILE '.../path/to/CovidVaccinations.csv'
...
-- Used new_cases > 0 in WHERE clause to filter out countries with no reported cases
...
-- This is very rough estimate only, not fully accurate. It doesn't account for people infected multiple times and population is constant for every date in this table
...
-- Filtered out aggregated entries like World, World excl. China .... using the WHERE clause
...
-- This data are not needed for now, so I specify which records I want by using a WHERE clause and IN() clause
...
SELECT *, 
	ROUND((cumulative_vaccinations/population)*100,2) AS doses_per_100_people
...
-- Vytvoření view pro uložení dat o procentu očkované populace
-- Window funkce SUM() OVER, která kumuluje počet očkování zemí v čase (ve vytvořeném view)

-- EN: Created view to store data of vaccination rates 
